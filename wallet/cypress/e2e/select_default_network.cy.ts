...
function verifyNetwork () : Promise<boolean>
{
    return new Promise((resolve, reject)  => {
        cy.contains("Something went wrong, Please Try Again!").should((_) => {}).then(($element) => {
            if (!($element || []).length) {
                resolve(true);
            } 
            else {
...
function networkSelectVerify() {
    return new Promise((resolve, reject) => {
        cy.get(".MuiList-root.MuiList-padding.MuiMenu-list.css-oz8p8s").children().each(($row: any) => {
            cy.get($row).invoke('text').then((textRow) => {
                if (textRow != "Add Custom Network") {
                    if(!networkTesteds.some((text) => text == textRow))
                    {
                        cy.get($row).invoke('attr', 'aria-selected').then((val: any) => {
                            if (val == "false") {
                                resolve($row);
                            }
                            else
